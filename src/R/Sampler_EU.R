# Perform sampling based on the EU-Thailand export model.
# The outbreak starts in 1980, with a growth rate of 6 to a population size of 20k individuals
# Then, in 1985, the growth rate drops to 2, and the population size increases to 30k individuals.
# In 1990, we mask down the population to 2k individuals randomly. This accounts for the fact that
# The Thai population must export sequences to the EU. This is a simplification, but it's very nice.
# We then draw samples, starting in 1991, in small amounts.
...
simulate_EU_Thai_HIV <- function(sample_size, ct_p, sample_times_data, sample_times_times, pop_size = 500,
                                 l1 = 5, l2 = 10, R0_init = 5, stretch = 0, init_pop_size = 100, R0 = 3,
                                 integer_sampling = FALSE, front_ratio = 20) {
    # Perform a simulation of the EU-Thailand export model.
    # Some of the growth parameters will be hard-coded here.
    ...
    first_constant_phase_endpoint <- 12 * l1
    second_constant_phase_endpoint <- 12 * l2
    ...
    if (integer_sampling == TRUE) {
        phylogeny <- SEEPS::stochastify_transmission_history(
                                transmission_history = phylogeny$phylogeny,
                                rate = 0.0067 / 12 * seq_len)
        phylogeny$phylogeny <- phylogeny$geneology  # Make a copy to fix the names
    }
    # print(phylogeny)
    # flush.console()
